...
This week I spent collecting the results of the LTASS experiments. I mainly focused on visualizing these results because soon it will be presentation season. In fact, one of the components I wanted to do better is visualizing these results excitingly and transparently. So, I'm shooting for something simple but profound! Descriptive art pieces? I also spent time brainstorming exciting ideas I can showcase speech recognition and highlight the relationship between speech recognition and language learning. Ultimately I did this because I think it will do a couple of things for me:
...
Hence, we're mainly interested in the average frequencies or spectrum of a particular voice (speech). We don't know if the average spectrum of the VoxForge and CHILDES corpora are different, but if they are significantly different, then perhaps some of the variability in performance can be attributed to their difference in the spectrum (or a combination of their frequencies, SNR, and other acoustical measurements).

Conceptually, the LTASS methods look like collecting all of the speech samples from a signal that was sampled at some sampling rate (let's say 16kHz). Then, taking the FFT/periodogram at some resolution. The resolution can be calculated beforehand because it depends on the FFT length and sampling rate:
